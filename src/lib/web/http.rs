...
use crate::services::{self,ServiceError,action};
...
pub async fn get_raw_clip(
    ...
    match action::get_clip(req, database.get_pool()).await {
        ...
        Err(e) => match e {
            ServiceError::PerissionError(msg) => Ok(status::Custom(Status::Unauthorized, msg)),
