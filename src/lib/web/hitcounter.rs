...
impl HitCounter {
    ...
    fn commit_hits(hits: HitStore, handle: Handle, pool: DatabasePool) -> Result<(), HitCountError> {
        ...
        handle.block_on(async move {
            ...
            let transaction = services::action::begin_transaction(&pool).await?;
            for (shortcode, hits) in hits {
                if let Err(e) = services::action::increase_hit_count(&shortcode, hits, &pool).await {
                    eprintln!("error increasing hit count: {}", e);
                }
            }
            Ok(services::action::end_transaction(transaction).await?)
        })
